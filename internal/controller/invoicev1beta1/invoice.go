...
func (c *external) updateStatus(cr *invoicev1beta1.Invoice, invoice *clients.Invoice) {
	cr.Status.AtProvider.ID = invoice.ID
...
func convertPaymentMethods(methods []clients.PaymentMethod) []invoicev1beta1.PaymentMethodDetails {
	result := make([]invoicev1beta1.PaymentMethodDetails, len(methods))
	for i, method := range methods {
		result[i] = invoicev1beta1.PaymentMethodDetails{
...
func convertPayments(payments []clients.Payment) []invoicev1beta1.PaymentDetails {
	result := make([]invoicev1beta1.PaymentDetails, len(payments))
	for i, payment := range payments {
		result[i] = invoicev1beta1.PaymentDetails{
			ID:          payment.ID,
			Value:       payment.Value,
			Fee:         payment.Fee,
